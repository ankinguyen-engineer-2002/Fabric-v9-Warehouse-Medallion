...
---------------------------------------------------------------------------------------------------------------------------*/
    ...
    BEGIN
        ...
        BEGIN TRY
            ...
            SET @StartDate = DATEADD(DAY,2290,CAST(@DateValue AS DATE));
            ...
            CREATE TABLE Placements_Wrk.ItemStatus_WOP
                (
                    Year             SMALLINT,
                    Week             INT,
                    WeekDate         DATE,
            ...
            INSERT INTO Placements_Wrk.ItemStatus_WOP
                (
                    Year,
                    Week,
                    WeekDate,
            ...
            DROP TABLE IF EXISTS Placements_Wrk.Weekly_History
            
            CREATE TABLE Placements_Wrk.Weekly_History
                (
                    [Year]     SMALLINT,
                    [Week]     INT,
                    WeekDate   DATE,
            ...
            INSERT INTO Placements_Wrk.Weekly_History
                (
                    Year,
                    Week,
                    WeekDate,
                    ...
                    Lost
                        ...
                        FROM
                            ...
                            JOIN
                               [$(MasterData_Warehouse)].MasterData_DW.DimDate_NonRetail WD
                                    ON D1.[FiscalYear] = WD.[FiscalYear]
                                       AND D1.[FiscalWeek] = WD.[FiscalWeek]
                                       AND DATEPART(dw, WD.DateID) = 1
                        WHERE
                                [OrderDate] >= @StartDate
                ...
                (
                    ...
                    Lost
                        ...
                        FROM
                            ...
                            JOIN
                                [$(MasterData_Warehouse)].MasterData_DW.DimDate_NonRetail D1
                                    ON DateID = CONVERT(DATE, CONVERT(CHAR(8), OrderDate))
                            JOIN
                                [$(MasterData_Warehouse)].MasterData_DW.DimDate_NonRetail WD
                                    ON D1.[FiscalYear] = WD.[FiscalYear]
                                       AND D1.[FiscalWeek] = WD.[FiscalWeek]
                        ...
                        FROM
                                (
                                    SELECT
                                            ...
                                            0         AS OrderAmt
                                    FROM
                                            Placements_Wrk.Weekly_History t1
                                        JOIN
                                            [$(MasterData_Warehouse)].MasterData_DW.DimDate_NonRetail
                                                ON DateID
                                                   BETWEEN DATEADD(DAY, 7, WeekDate) AND DATEADD(DAY, 84, WeekDate)
                                                   AND DATEPART(dw, DateID) = 1
                                    WHERE
                                            OrderQty > 0
                                )               t4
                            LEFT JOIN
                                Placements_Wrk.Weekly_History t2
                        ...
                        FROM
                                (
                                    ...
                                    FROM
                                            Placements_Wrk.Weekly_History             t1
                                        JOIN
                                            Placements_Wrk.ItemStatus_WOP T3
                        ...
                        FROM
                                (
                                    ...
                                    FROM
                                        ...
                                        JOIN
                                            [$(MasterData_Warehouse)].MasterData_DW.DimDate_NonRetail
                                                ON DateID = DATEADD(DAY,7 ,WeekDate)
                                    WHERE
                                            Placement > 0
                                )               t4
                            LEFT JOIN
                                Placements_Wrk.Weekly_History FW
            ...
            CREATE TABLE Placements.[CustItemWeeklyPlacements_LOAD]
                (
            ...
            INSERT INTO Placements.CustItemWeeklyPlacements_LOAD
                (
                    CustomerNumber,
                    ShiptoNumber,
                    ItemSKU,
                    ...
                    Week
                        ...
                        SELECT
                                ...
                                h.Week
                        FROM
                                Placements_Wrk.Weekly_History                                       h
                            JOIN
                                Placements_Wrk.ItemStatus_WOP                           i
                            ...
                            LEFT JOIN
                                Placements_Wrk.Weekly_History                                       PW
                                    ON PW.WeekDate = DATEADD(DAY, -7,CAST(h.WeekDate AS DATE))
                                       AND h.CustomerNumber = PW.CustomerNumber
                                       AND h.ShiptoNumber = PW.ShiptoNumber
                                       AND h.ItemSKU = PW.ItemSKU
                        WHERE
                                h.WeekDate
                        BETWEEN DATEADD(DAY, -2190,CAST(@DateValue AS DATE)) AND @DateValue;



            CREATE STATISTICS [Stat_CustItemWeeklyPlacements_CustomerNumber]
                ON Placements.[CustItemWeeklyPlacements_LOAD]
                (
                    [CustomerNumber]
                );
            ...
            DROP TABLE IF EXISTS
                Placements.CustItemWeeklyPlacements;

            EXECUTE sp_rename 'Placements.CustItemWeeklyPlacements_LOAD','CustItemWeeklyPlacements'

  
            DROP TABLE Placements_Wrk.Weekly_History;
        ...
        VALUES
            (
               'AFISales_DW', 'Placements', 'CustItemWeeklyPlacements', @DateValue
            );
