CREATE                   PROCEDURE [Retail_DW_Core].[usp_Refresh_StorePulse_CEMT]
AS
...
BEGIN
	...
	SET @String = '[Retail_DW_Core].[usp_Refresh_StorePulse_CEMT]';
	SET @User = SYSTEM_USER;
	SET @DateValue = GETDATE();
	SET @DestinationDatabase = 'Retail_Warehouse';
	SET @DestinationSchema = 'Retail_DW_Core';
	SET @DestinationTable = 'FactStorePulseCEMT';
...
BEGIN TRY
    ...
    Truncate TABLE [Retail_DW_Core].[FactStorePulseCEMT]

	DROP TABLE IF exists [Retail_DW_Core].[FactStorePulseCEMTHolding]
	CREATE TABLE [Retail_DW_Core].[FactStorePulseCEMTHolding]
(
...
into #wrk_Store_Date
FROM 
    (select distinct StoreID from Retail_DW_Core.DimStoreLocation WHERE LocationType IN ('ST','WH')) A  -- Added Warehouse for bring all store ids for location groups 
CROSS JOIN 
...
-- Final table
--select top 2 * from [Retail_DW_Core].[FactStorePulseCEMTHolding]

INSERT INTO [Retail_DW_Core].[FactStorePulseCEMTHolding]
(
LocationKey
,OrderChangeDate
,Written_GrossSale
...
-- into Retail_DW_Core.FactStorePulseCEMTHolding
...
DELETE FROM Retail_DW_Core.FactStorePulseCEMTHolding WHERE LocationKey in (111, 613, 627);-- , 332);

-- Drop table if exists Retail_DW_Core.FactStorePulseCEMT
INSERT INTO [Retail_DW_Core].[FactStorePulseCEMT]
(
Division
,Region
,DateKey
...
SELECT 
	...
	, a.DerivedUpsBudget
	, a.CloseGoalBudget
	-- into Retail_DW_Core.FactStorePulseCEMT
FROM Retail_DW_Core.FactStorePulseCEMTHolding a 
	...
	LEFT JOIN Retail_DW_Core.DimRollUps c ON b.StoreID = c.StoreID 
		AND (
			(c.[RollUpFilter] = 'Division' AND c.[RollUp] NOT IN ('WDIV','EDIV')) 
			OR (c.StoreID = 642 AND c.[RollUp] = 'ALSTR')
		)
	LEFT JOIN (select distinct StoreID from Retail_DW_Core.DimStoreLocationGroup where LocationGroupID IN ('A-P-US', 'A-V-US', 'A-V-Ashcom', 'A-V-Eteam', 'O-P-US', 'W-P-US', 'ALL-US')) d 
			ON b.StoreID = d.StoreID 
WHERE 
	-- Include stores from DimRollUps with your existing filter
	(c.StoreID IS NOT NULL)
	OR 
	-- Include stores from DimStoreLocationGroup that are NOT in DimRollUps
	(c.StoreID IS NULL AND d.StoreID IS NOT NULL)



update a set a.Region = b.[RollUp] 
from Retail_DW_Core.FactStorePulseCEMT a 
join Retail_DW_Core.DimRollUps b on a.LocationKey = b.StoreID
where b.RollUpFilter = 'Region'

DROP TABLE [Retail_DW_Core].[FactStorePulseCEMTHolding];
