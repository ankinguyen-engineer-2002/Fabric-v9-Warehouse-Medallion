...
![SEMANTIC](https://img.shields.io/badge/SEMANTIC_MODEL-Direct_Lake-EC4899?style=flat-square&logo=powerbi&logoColor=white)
![MATURITY](https://img.shields.io/badge/MATURITY-90.0%25-22C55E?style=flat-square)
...
## Semantic Model — `sc_forecast_control_tower`

Live Power BI semantic model on top of Gold WH using **Direct Lake** mode.

| Attribute | Value |
|---|---|
| Display name | `sc_forecast_control_tower` |
| ID | `f06a2361-15fd-4f91-9d37-941fefe62aaf` |
| Source | `SupplyChain_Gold_Warehouse` (Direct Lake) |
| Schema | `ForecastAccuracy_DW` |
| Tables | 7 (5 Dim + 2 Fact) + `_Measure` + 2 parameter tables |
| DAX measures | 35 (MAPE, RMSE, Bias, Tracking signal, ...) |
| Relationships | 9 |
| RLS role | `All_Permission` |
| Deployed | 2026-05-05 — see [ADR-007](docs/decisions/ADR-007-v10-semantic-model-deployment.md) |

Cloned from Cherry/BCherry's `Supply Chain Control Tower` (v8 Lakehouse/Spark) after closing v8↔v10 ETL parity gaps:
- DimCalendar: 10 → **74 cols** (extended Silver `vw_Calendar` from `Enterprise_Lakehouse.MasterData_DW.DimDate`)
- FactForecastKpi: 12 → **18 cols** (added 7 derived metrics: `QtyNaiveFcstError`, `QtyAbsNaiveFcstError`, `QtySquaredFcstError`, `QtySquaredNaiveFcstError`, `ValidObsFlag`, `ValidActualNonzeroFlag`, `AbsPctError`)
- DimForecastHorizon: 2 → **3 cols** (+`Rank` for sort order)

See [doc 17 — v8→v10 ETL Parity](02_Architect_v10_May/30_runbook/17_v8_to_v10_etl_parity.md) and [ADR-007](docs/decisions/ADR-007-v10-semantic-model-deployment.md).

### Lineage extension — Gold → SemanticModel edges

`Meta.LineageEdge` extended to track which semantic model consumes each Gold table. Live state:
...
Manual refresh when adding/repointing models:
```bash
python3 02_Architect_v10_May/tools/build_semantic_model_lineage.py
```

`Meta.usp_BuildLineage` is guarded so daily pipeline rebuild preserves semantic edges (see [doc 18](02_Architect_v10_May/30_runbook/18_lineage_extension_to_semantic_models.md) for full architecture + IT permission requirements for full automation).
