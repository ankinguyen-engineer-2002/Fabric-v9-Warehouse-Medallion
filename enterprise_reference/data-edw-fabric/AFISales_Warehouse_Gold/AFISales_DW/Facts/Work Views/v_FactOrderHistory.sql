CREATE VIEW [AFISales_DW_Wrk].[v_FactOrderHistory]
AS
    SELECT
            ROW_NUMBER() OVER(ORDER BY  OrderHistory.[OrderNumber]) AS RowID,
            ...
            [Amount Cancelled]                = -1.00 * CAST((CASE
                                                                  WHEN OrderHistory.[Quantity] < 0
                                                                       AND ISNULL(OrderCancellationReasonCode.CancelCategory, 'SYSTEMIC') <> 'SYSTEMIC'
                                                                      THEN
            ([NetAmount] - (OrderHistory.[Quantity] * OrderHistory.Freight) - (OrderHistory.[Quantity] * OrderHistory.HiddenFreight) + (OrderHistory.[Quantity] * OrderHistory.Discount))
                                                                  ELSE
                                                                      0
                                                              END
                                                             ) * ISNULL(TerritoryAllocationStatic.CommissionSplitPercent, 1) AS DECIMAL(13, 3)),
            [Quantity Ordered]                = CAST((OrderHistory.[Quantity] * ISNULL(TerritoryAllocationStatic.CommissionSplitPercent, 1)) AS DECIMAL(13, 3)),
            [Amount Ordered]                  = CAST((OrderHistory.[NetAmount] - (OrderHistory.[Quantity] * OrderHistory.Freight) - (OrderHistory.[Quantity] * OrderHistory.HiddenFreight)
                                                      + (OrderHistory.[Quantity] * OrderHistory.Discount)
                                                     ) * ISNULL(TerritoryAllocationStatic.CommissionSplitPercent, 1) AS DECIMAL(13, 3)),
            [Order Freight]                   = CAST(OrderHistory.Freight * OrderHistory.[Quantity] * ISNULL(TerritoryAllocationStatic.CommissionSplitPercent, 1) AS DECIMAL(13, 3)),
            [Total Freight]                   = CAST(((OrderHistory.Freight * OrderHistory.[Quantity]) + (OrderHistory.HiddenFreight * OrderHistory.[Quantity])) * ISNULL(TerritoryAllocationStatic.CommissionSplitPercent, 1) AS DECIMAL(13, 3)),
            [Allocated Order Freight]         = CAST((OrderHistory.HiddenFreight * OrderHistory.[Quantity]) * ISNULL(TerritoryAllocationStatic.CommissionSplitPercent, 1) AS DECIMAL(13, 3)),
            [Order Discounts]                 = CAST((OrderHistory.[Quantity] * OrderHistory.Discount) * ISNULL(TerritoryAllocationStatic.CommissionSplitPercent, 1) AS DECIMAL(13, 3)),
            ...
            [Quantity Cancelled]              = -1.00 * CAST((CASE
                                                                  WHEN [Quantity] < 0
                                                                       AND ISNULL(OrderCancellationReasonCode.CancelCategory, 'SYSTEMIC') <> 'SYSTEMIC'
                                                                      THEN
            ...
            [Reason Code]                     = ISNULL(OrderCancellationReasonCode.ReasonCode + ' - ' + OrderCancellationReasonCode.ReasonDescription, 'N/A'),
    ...
    FROM
        ...
        LEFT JOIN
            [$(Wholesale_Warehouse)].Marketing.Regions
                ON TerritoryAllocationStatic.RegionCode = Regions.RegionCode 
        JOIN
        ...
        LEFT JOIN
            AFISales_DW.[DimSalesTerritories]       
                ON DimSalesTerritories.[AFI Sales Region Code] = ISNULL(TerritoryAllocationStatic.RegionCode, CAST('Z' AS CHAR(3)))
                   ...
                   AND DimSalesTerritories.[AFI Sales Category] = ISNULL(DimItemMaster.AFISalesCategoryCode, CAST('ZZ' AS CHAR(3)))
