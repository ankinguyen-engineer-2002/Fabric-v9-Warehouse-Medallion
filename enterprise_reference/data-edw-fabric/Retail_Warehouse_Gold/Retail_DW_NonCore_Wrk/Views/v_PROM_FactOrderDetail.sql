CREATE VIEW [Retail_DW_NonCore_Wrk].[v_PROM_FactOrderDetail]
AS
   SELECT 
    O.SourceOrderID,
    O.SKU,
    ...
    --         CASE
    --             WHEN O.UnitSellPrice < O.UnitPromoPrice THEN 1
    --             ELSE 0
    --         END
    --     ELSE 0
    -- END AS SellPriceLTPromoFlag,
    O.StoreID,
    OH.SalesPersonID,
    S2.SalesPersonName,
...
WHERE 
    ...
    AND O.TransCodeID IN (0, 1, 7) AND G.CategoryID IN ('ACCESS','BEDDI','BEDRO','CASEG','DININ','MOTION','OUTDR','UPHOL','WARRI','WARRT')
GO
