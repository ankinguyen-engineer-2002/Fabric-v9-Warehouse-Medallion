CREATE VIEW AFISales_DW_Wrk.v_FactOpenOrders_AcctOwnership
AS
    SELECT
            ...
            [Open Order Amount]                        = CAST(Orders.OpenOrderAmt * ISNULL(MrktSpclstAcctOwnershipSlsCat.Ratio, 1) AS DECIMAL(13, 3)),
            [Open Order Quantity]                      = CAST(Orders.OpenOrderQty * ISNULL(MrktSpclstAcctOwnershipSlsCat.Ratio, 1) AS DECIMAL(13, 3)),
            [Back Order Amount]                        = CAST(Orders.BackOrderAmt * ISNULL(MrktSpclstAcctOwnershipSlsCat.Ratio, 1) AS DECIMAL(13, 3)),       --AS MONEY
            [Order Arrival Mode]                       = Orders.OrderArrival,
            [Back Order Quantity]                      = CAST(Orders.BackOrderQty * ISNULL(MrktSpclstAcctOwnershipSlsCat.Ratio, 1) AS DECIMAL(13, 3)),
            ...
            [Open Order Discounts]                     = CAST((Orders.[Order Discount]) * ISNULL(MrktSpclstAcctOwnershipSlsCat.Ratio, 1) AS DECIMAL(13, 3)), -- Rev 10
            [Open Order Freight]                       = CAST((Orders.[Order Freight] * Orders.OpenOrderQty)
                                                              * ISNULL(MrktSpclstAcctOwnershipSlsCat.Ratio, 1) AS DECIMAL(13, 3)),                           -- Rev 10


            [Trip Number(s)]                           = CAST(Orders.TripNo AS VARCHAR(650)),
            [Customer PO]                              = Orders.PurchaseOrder,
            [Account Exception Flag]                   = Orders.[Account Exception Flag],
            [Order Freight]                            = Orders.[Order Freight]
    FROM
            (
                SELECT
                        ...
                        DimCustomers.[Account Exception Flag]
--                        OpenOrderExtendedItem.Freight as [Order Freight]
                FROM
        ...
        LEFT JOIN
            AFISales_Enh.MrktSpclstAcctOwnershipSlsCat
                ON Orders.Division = MrktSpclstAcctOwnershipSlsCat.Division
                   AND Orders.CustomerNumber = MrktSpclstAcctOwnershipSlsCat.CustomerNumber
                   AND CASE
                   ...
                   AND Orders.SalesCategory = MrktSpclstAcctOwnershipSlsCat.SalesCategory
        LEFT JOIN
