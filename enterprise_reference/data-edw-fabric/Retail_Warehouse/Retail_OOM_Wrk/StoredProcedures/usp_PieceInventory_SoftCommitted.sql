CREATE   PROCEDURE [Retail_OOM_Wrk].[usp_PieceInventory_SoftCommitted] @TransDate DATETIME
AS
BEGIN
    ...
    SET @String = 'Retail_OOM_wrk.usp_PieceInventory_SoftCommitted';
    ...
    BEGIN TRY
        ...
        AS
        (
            SELECT  oi.LocationID AS StoreID,
                    '01' AS SourceID,
                    oi.ProductID,
                    SUM(oi.QtyCommitted) AS TotalCommitted
            FROM [Retail_OOM_Enh].BucketOrders AS oi
                INNER JOIN [$(Source_Data)].[Retail_Corporate].[Product] AS p
                    ON oi.ProductID = p.ProductID
            WHERE oi.TransCodeID IN (0, 7, 60, 61, 63, 66, 67)
                  AND p.ProductTypeID = '1'
                  AND p.SpecialOrder = 0
                  AND oi.QtyCommitted <> 0
                  AND oi.OrderType = 'D'
            GROUP BY oi.LocationID,
                     oi.ProductID
        ),
        HardCommitted
        AS
