CREATE PROCEDURE [Retail_DW_NonCore].[usp_Update_FactOOMSchedulePerformance]
AS
...
BEGIN
	...
	SET @String = 'Retail_DW_NonCore.usp_Update_FactOOMSchedulePerformance';
	SET @User = SYSTEM_USER;
	SET @DateValue = GETDATE();
	SET @DestinationDatabase = 'Retail_Warehouse';
	SET @DestinationSchema = 'Retail_DW_NonCore';
	SET @DestinationTable = 'FactOOMSchedulePerformance';
	...
	BEGIN TRY
		...
		--TRUNCATE TABLE [Retail_DW_NonCore].[FactOOMSchedulePerformance];

		DECLARE @TransDate DATE = (GETDATE() - 1);

	    DROP TABLE IF EXISTS [Retail_DW_NonCore].[FactOOMSchedulePerformanceHolding];

		CREATE TABLE [Retail_DW_NonCore].[FactOOMSchedulePerformanceHolding]
		(
		...
		INSERT INTO [Retail_DW_NonCore].[FactOOMSchedulePerformanceHolding]
		(	
			TransDate
			, WrittenSales
			, TotalScheduleStore
		...
		SELECT 
			...
			, TotalScheduleSMSOrderCount
			, TotalScheduleChatBotOrderCount
		FROM [$(Retail_Warehouse)].[Retail_OOM_Enh].[OOMSchedulePerformance]
		WHERE TransDate = @TransDate;

		DELETE FROM [Retail_DW_NonCore].[FactOOMSchedulePerformance]
		WHERE TransDate = @TransDate;

		INSERT INTO [Retail_DW_NonCore].[FactOOMSchedulePerformance]
		(
			TransDate
			, WrittenSales
			, TotalScheduleStore
		...
		SELECT 
			...
			, TotalScheduleSMSOrderCount
			, TotalScheduleChatBotOrderCount
		FROM [Retail_DW_NonCore].[FactOOMSchedulePerformanceHolding];

	    DROP TABLE [Retail_DW_NonCore].[FactOOMSchedulePerformanceHolding];
